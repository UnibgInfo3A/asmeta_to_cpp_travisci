...
namespace Hemodialysis_ref4_forMCnamespace{
	enum PhasesTherapy {PREPARATION, INITIATION, ENDING};
	
	enum ModePreparation {RINSE_DIALYZER, SET_TREAT_PARAM, PREPARE_HEPARIN, TUBING_SYSTEM, SET_RINSING_PARAM, CONNECT_CONCENTRATE, AUTO_TEST};
	
	enum ModeInitiation {CONNECT_PATIENT, THERAPY_RUNNING};
	
	enum ModeEnding {REINFUSION, THERAPY_OVERVIEW, EMPTY_CARTRIDGE, DRAIN_DIALYZER};
	
	enum RinsingParam {FILLING_BP_RATE, FILLING_BP_VOLUME, BP_RATE_RINSING, DF_FLOW_RINSING, TIME_RINSING, UF_RATE_RINSING, UF_VOLUME_RINSING};
	
	enum TubingPhase {CONNECT_AV_TUBES, CONNECT_ALL_COMP, SET_SALINE_LEVELS, INSERT_BLOODLINES, PRIMING, CONNECT_AV_ENDS};
	
	enum TreatmentParam {BLOOD_CONDUCTIVITY, BIC_AC, BIC_CONDUCTIVITY, DF_TEMP, DF_FLOW, UF_VOLUME, THERAPY_TIME, MIN_UF_RATE, MAX_UF_RATE, MIN_AP, MAX_AP, MIN_VP, MAX_VP, DELTA_AP, PERC_DELTA_TMP, LIMITS_TMP, MAX_TMP, EXTENDED_TMP, MAX_BEP, STOP_TIME_H, BOLUS_VOLUME_H, RATE_H, ACTIVATION_H, SYRINGE_TYPE};
	
	enum RinsePhase {CONNECT_DIALYZER, FILL_ART_CHAMBER, FILL_VEN_CHAMBER, FILL_DIALYZER};
	
	enum PatientPhase {CONN_ART, START_BP, BLOOD_FLOW, FILL_TUBING, CONN_VEN, END_CONN};
	
	enum TherapyPhase {START_HEPARIN, THERAPY_EXEC, THERAPY_END};
	
	enum ReinfusionPhase {REMOVE_ART, CONN_SALINE, START_SALINE_INF, RUN_SALINE_INF, CHOOSE_NEXT_REINF_STEP, START_SALINE_REIN, RUN_SALINE_REIN, REMOVE_VEN};
	
	enum BPStatus {START, STOP};
	
	enum SignalLamps {GREEN, YELLOW};
	
	enum MachineState {BYPASS, MAIN_FLOW};
	
	enum ArterialBolusPhase {WAIT_SOLUTION, SET_ARTERIAL_BOLUS_VOLUME, CONNECT_SOLUTION, RUNNING_SOLUTION};
	
	enum ErrorAlarmType {UF_BYPASS, UF_VOLUME_ERR, UF_DIR, UF_RATE, SAD_ERR, HEPARIN_DIR, DF_PREP, FILL_BLOOD_VOL, REIN_VP_UP, REIN_AP_LOW, CONN_VP_UP, CONN_VP_LOW, CONN_AP_LOW, INIT_VP_UP, INIT_VP_LOW, INIT_AP_UP, INIT_AP_LOW, BP_NO_FLOW, BP_LESS_FLOW, BP_ROTATION_DIR, TEMP_HIGH, TEMP_LOW, ARTERIAL_BOLUS, ARTERIAL_BOLUS_END};
...
const std::set<PhasesTherapy> PhasesTherapy_elems;

const std::set<ModePreparation> ModePreparation_elems;

const std::set<ModeInitiation> ModeInitiation_elems;

const std::set<ModeEnding> ModeEnding_elems;

const std::set<RinsingParam> RinsingParam_elems;

const std::set<TubingPhase> TubingPhase_elems;

const std::set<TreatmentParam> TreatmentParam_elems;

const std::set<RinsePhase> RinsePhase_elems;

const std::set<PatientPhase> PatientPhase_elems;

const std::set<TherapyPhase> TherapyPhase_elems;

const std::set<ReinfusionPhase> ReinfusionPhase_elems;

const std::set<BPStatus> BPStatus_elems;

const std::set<SignalLamps> SignalLamps_elems;

const std::set<MachineState> MachineState_elems;

const std::set<ArterialBolusPhase> ArterialBolusPhase_elems;

const std::set<ErrorAlarmType> ErrorAlarmType_elems;

const std::set<SADLimit> SADLimit_elems;

const std::set<LMHlimit> LMHlimit_elems;

const std::set<Airlimit> Airlimit_elems;
...
std::map<ErrorAlarmType, bool> error[2];
std::map<ErrorAlarmType, bool> alarm[2];
bool reset_alarm;
...
void r_insert_param (int _inf, int _sup, RinsingParam _nextparam, int _mon, int _contr);
void r_insert_param (int _inf, int _sup, ModePreparation _nextparam, int _mon, int _contr);
void r_insert_param_press (int _inf, int _sup, int _monmin, int _contrmin, int _monmax, int _contrmax, bool _succ);
void r_insert_param (int _inf, int _sup, TreatmentParam _nextparam, int _mon, int _contr);
void r_insert_param (int _inf, int _sup, PatientPhase _nextparam, int _mon, int _contr);
...
void r_check_init_press_limit (ErrorAlarmType _err, LMHlimit _limit, bool _errbf, bool _errpress);
void r_check_init_vp_up_limit();
void r_check_init_ap_up_limit();
void r_check_init_vp_low_limit();
void r_check_init_ap_low_limit();
void r_check_conn_rein_press_limit (ErrorAlarmType _err, LMHlimit _limit, bool _passedT);
void r_check_conn_rein_press_limit (ErrorAlarmType _err, LMHlimit _limit, bool _passedT, bool _errbf, bool _errpress);
